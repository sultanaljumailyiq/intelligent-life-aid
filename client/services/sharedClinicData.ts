...
class SharedClinicDataService {
  ...
  async addClinic(clinicData: any): Promise<Clinic> {
    const newClinic: Clinic = {
      ...
      onlineBookingEnabled: clinicData.onlineBookingEnabled || false,
      bookingLink: `/simplified-booking/clinic-${Date.now()}`,
      workingHours: {},
      timeSlotDuration: 30,
      breakTimes: [],
      acceptedTreatments: []
  ...
  async updateClinic(clinicId: string, clinicData: any): Promise<Clinic | null> {
    const index = this.clinics.findIndex((c) => c.id === clinicId);
    if (index !== -1) {
      this.clinics[index] = {
        ...this.clinics[index],
        ...clinicData,
        id: clinicId // Preserve ID
      };
