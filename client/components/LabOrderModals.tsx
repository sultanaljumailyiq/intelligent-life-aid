...
import { createRemindersForLabOrder } from "@/services/ReminderAutomationService";
import {
  syncLabOrderToClinicDashboard,
  syncLabOrderToDentalHub,
  notifyStaff,
...
interface LabOrderModalProps {
  ...
  preFilledToothData?: ToothData; // Pre-filled from dental chart selection
}
...
export function NewLabOrderModal({
  ...
  // Auto-populate from dental chart selection
  useEffect(() => {
    if (preFilledToothData && isOpen) {
      setTreatment(preFilledToothData.treatment);
      setTooth(preFilledToothData.number.toString());
      if (preFilledToothData.notes) {
        setNotes(preFilledToothData.notes);
      }
    }
  }, [preFilledToothData, isOpen]);
  ...
  const handleSubmit = async () => {
    try {
      ...
      await createRemindersForLabOrder({
        id: createdLabOrder.id,
      ...
      // Sync to clinic dashboard
      await syncLabOrderToClinicDashboard({
        id: createdLabOrder.id,
        patientId: selectedPatient,
        patientName: selectedPatientData?.name || "",
        labId: selectedLab,
      ...
      // Auto-create treatment plan step if not already exists
      if (selectedPatientData) {
        try {
          await fetch(`/api/patients/${selectedPatient}/treatment-plan`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
            body: JSON.stringify({
              treatment,
              tooth,
              status: "ordered_from_lab",
              linkedLabOrderId: createdLabOrder.id,
              createdAt: new Date().toISOString(),
            }),
          });
        } catch (error) {
          console.warn("Failed to auto-create treatment plan step:", error);
        }
      }
      ...
      alert("حدث خطأ عند إنشاء طلب المختبر. يرجى المحاولة مجددا.");
    }
  };
